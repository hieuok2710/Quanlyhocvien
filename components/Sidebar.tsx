...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, userProfile, onLogout }) => {
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 dark:bg-[#0B1121]/90 backdrop-blur-xl border-t border-slate-200 dark:border-white/5 pb-safe-area pt-2 px-4 z-50 flex justify-between items-center shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
        {menuItems.map((item) => {
          const Icon = item.icon;
          const isActive = currentView === item.id;
          const theme = menuConfig[item.id];
          ...
          return (
            <button
              key={item.id}
              onClick={() => onChangeView(item.id as ViewState)}
              className={`flex flex-col items-center justify-center p-2 rounded-2xl transition-all duration-300 w-14 ${
                isActive ? '' : 'opacity-60 hover:opacity-100'
              }`}
            >
              <div className={`p-1.5 rounded-xl transition-all duration-300 mb-1 ${
                isActive 
                  ? `${theme.activeBg} ${theme.color} border ${theme.activeBorder} shadow-lg ${theme.activeShadow} -translate-y-2` 
              ...
              <span className={`text-[9px] font-bold tracking-wide transition-all ${
                 isActive 
                   ? `opacity-100 translate-y-0 ${theme.color}` 
                   : 'opacity-0 translate-y-2 hidden'
              }`}>
                {item.label}
              </span>
              {!isActive && <div className="w-1 h-1 rounded-full bg-transparent mb-[9px]" />}
            </button>
          )
        })}
      </div>
