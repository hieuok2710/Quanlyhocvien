...
interface SidebarProps {
  ...
  isMobile?: boolean; // New prop to force mobile layout
}
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, userProfile, isMobile = false }) => {
  ...
  return (
    <>
      {/* DESKTOP SIDEBAR (Hidden on mobile or when isMobile prop is true) */}
      {!isMobile && (
        <div className="hidden md:flex h-screen w-64 bg-white dark:bg-[#0B1121] flex-col shadow-2xl z-20 relative overflow-hidden border-r border-slate-200 dark:border-white/5 transition-colors duration-300">
          ...
          <div className="p-6 flex items-center gap-3 relative z-10">
            ...
            <div>
              <h1 className="text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-900 via-slate-700 to-slate-500 dark:from-white dark:via-slate-200 dark:to-slate-400 tracking-tight">
                EduNova
              </h1>
          ...
          {/* Navigation - Compact */}
          <nav className="flex-1 px-3 space-y-1 mt-1 overflow-y-auto custom-scrollbar relative z-10">
            <p className="px-4 text-[10px] font-bold text-slate-500 dark:text-slate-600 uppercase tracking-wider mb-2">Menu Chính</p>
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <div className={`absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-6 rounded-r-full bg-current opacity-50`} />
                  )}
                  ...
                  />
                  <span className={`text-sm font-medium tracking-wide ${isActive ? '' : `text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-200`}`}>
                    {item.label}
                  </span>
          ...
          {/* Footer - Compact */}
          <div className="p-3 mx-2 mb-4 mt-auto relative z-10">
      ...
      {/* MOBILE BOTTOM NAVIGATION (Visible on Mobile OR when isMobile prop is true) */}
      <div className={`${isMobile ? 'flex' : 'md:hidden'} fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-dark-900/95 backdrop-blur-md border-t border-slate-200 dark:border-dark-700 pb-safe z-50 transition-all duration-300 ${isMobile ? 'absolute' : ''}`}>
        <div className="flex justify-around items-center h-16 px-2 w-full">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                {isActive && (
                   <div className="absolute top-0 w-8 h-0.5 rounded-b-full bg-current"></div>
                )}
                
                <div className={`p-1.5 rounded-full transition-all duration-300 ${
                  isActive ? `${theme.activeBg} transform -translate-y-1 shadow-sm` : ''
                }`}>
                  <Icon className={`w-5 h-5 ${isActive ? 'stroke-[2.5px]' : 'stroke-2'}`} />
                </div>
                <span className={`text-[10px] font-medium transition-colors duration-300 ${isActive ? 'opacity-100' : 'opacity-70'}`}>
                  {item.label}
                </span>
              </button>
            );
