...
const Dashboard: React.FC<DashboardProps> = ({ students, isMobile = false }) => {
  ...
  return (
    ...
    >
      ...
      {/* Content Wrapper */}
      <div className={`relative z-10 space-y-4 ${isMobile ? 'p-3' : 'p-8 space-y-8'}`}>
        <header className={`flex ${isMobile ? 'flex-col gap-2' : 'flex-col md:flex-row justify-between items-start md:items-end'} border-b border-slate-200 dark:border-dark-700 pb-2 md:pb-6`}>
          <div>
            <h2 className={`${isMobile ? 'text-xl' : 'text-3xl'} font-bold text-slate-900 dark:text-white mb-0.5`}>Tổng Quan</h2>
            {!isMobile && <p className={`text-slate-500 dark:text-slate-400 ${isMobile ? 'text-xs' : 'text-base'}`}>Báo cáo hiệu suất và thống kê hệ thống</p>}
          </div>
          <div className={`flex items-center gap-4 bg-white dark:bg-dark-800/50 px-3 py-2 md:p-3 rounded-xl border border-slate-200 dark:border-dark-700/50 backdrop-blur-sm shadow-lg ${isMobile ? 'w-full justify-between' : ''}`}>
            {isMobile && <span className="text-[10px] font-bold uppercase text-primary">Live Stats</span>}
            <div className="text-right flex items-center gap-3">
              <div className={`flex items-center justify-end gap-1.5 text-primary font-bold ${isMobile ? 'text-base' : 'text-xl'} font-mono tracking-widest`}>
                  <Clock className={`${isMobile ? 'w-3.5 h-3.5' : 'w-5 h-5'} mb-0.5`} />
        ...
        {/* Stat Cards - Ultra Compact Grid for Mobile */}
        <div className={`grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 ${isMobile ? 'gap-2' : 'gap-6'}`}>
          <StatCard
            title="Tổng số HV"
        ...
        <div className={`grid grid-cols-1 ${isMobile ? 'gap-4' : 'xl:grid-cols-3 gap-8'}`}>
          
          {/* Top Performers (Leaderboard) */}
          <div className={`${isMobile ? '' : 'xl:col-span-2'} bg-white dark:bg-gradient-to-br dark:from-dark-800 dark:to-dark-900 border border-slate-200 dark:border-dark-700 rounded-2xl ${isMobile ? 'p-3' : 'p-6'} shadow-xl relative overflow-hidden group/card hover:border-primary/20 transition-colors`}>
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover/card:opacity-20 transition-opacity duration-500">
              <Trophy className={`${isMobile ? 'w-20 h-20' : 'w-32 h-32'} text-yellow-500`} />
            </div>
            
            <h3 className={`${isMobile ? 'text-sm' : 'text-lg'} font-bold text-slate-900 dark:text-white mb-3 flex items-center gap-2 relative z-10`}>
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 relative z-10 ${isMobile ? 'grid-cols-2' : ''}`}>
              {topStudents.map((student, index) => (
                <div key={student.id} className={`bg-slate-50 dark:bg-dark-700/50 backdrop-blur-md border border-slate-200 dark:border-white/5 ${isMobile ? 'p-2' : 'p-4'} rounded-xl flex flex-col items-center text-center hover:shadow-lg dark:hover:bg-dark-700 hover:border-primary/30 transition-all group cursor-default`}>
                  <div className="relative mb-2">
                    <img src={student.avatar} alt={student.name} className={`${isMobile ? 'w-10 h-10' : 'w-16 h-16'} rounded-full border-2 border-slate-300 dark:border-dark-600 object-cover shadow-lg group-hover:scale-105 transition-transform`} />
                    <div className={`absolute -top-1 -right-1 ${isMobile ? 'w-4 h-4 text-[9px]' : 'w-6 h-6 text-xs'} rounded-full flex items-center justify-center font-bold shadow-lg border border-white dark:border-dark-800 ${
                  ...
                  <h4 className={`font-bold text-slate-800 dark:text-white ${isMobile ? 'text-[10px]' : 'text-sm'} truncate w-full mb-0.5`}>{student.name}</h4>
                  <div className="mt-auto px-1.5 py-0.5 bg-white dark:bg-white/5 rounded-full border border-slate-200 dark:border-white/5">
                    <span className={`font-bold text-primary ${isMobile ? 'text-[10px]' : 'text-sm'}`}>{student.gpa}</span> <span className="text-[8px] md:text-[10px] text-slate-500">GPA</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className={`bg-white dark:bg-dark-800 border border-slate-200 dark:border-dark-700 rounded-2xl ${isMobile ? 'p-3' : 'p-6'} shadow-xl flex flex-col hover:border-primary/20 transition-colors`}>
            <h3 className={`${isMobile ? 'text-sm' : 'text-lg'} font-semibold text-slate-900 dark:text-white mb-3 flex items-center gap-2`}>
              <Clock className="w-4 h-4 text-primary" /> Hoạt động mới
            </h3>
            
            <div className={`relative pl-3 border-l border-slate-200 dark:border-dark-700 ${isMobile ? 'space-y-4' : 'space-y-8'}`}>
              {[
                ...
                <div key={i} className="relative group cursor-pointer">
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className={`font-bold text-slate-700 dark:text-slate-200 group-hover:text-primary transition-colors ${isMobile ? 'text-[11px]' : 'text-sm'}`}>{item.title}</h4>
                      <p className={`text-slate-500 dark:text-slate-400 mt-0.5 ${isMobile ? 'text-[9px]' : 'text-xs'}`}>{item.desc}</p>
                    </div>
        ...
        <div className={`grid grid-cols-1 ${isMobile ? 'gap-4' : 'lg:grid-cols-3 gap-6'}`}>
          {/* Main Area Chart */}
          <div className={`${isMobile ? '' : 'lg:col-span-2'} bg-white dark:bg-dark-800 border border-slate-200 dark:border-dark-700 rounded-2xl ${isMobile ? 'p-3' : 'p-6'} shadow-xl hover:border-primary/20 transition-colors`}>
            <div className="flex justify-between items-center mb-4">
              <h3 className={`${isMobile ? 'text-sm' : 'text-lg'} font-semibold text-slate-900 dark:text-white`}>Hiệu suất Học tập</h3>
            </div>
          ...
          {/* Pie Chart */}
          <div className={`bg-white dark:bg-dark-800 border border-slate-200 dark:border-dark-700 rounded-2xl ${isMobile ? 'p-3' : 'p-6'} shadow-xl flex flex-col hover:border-primary/20 transition-colors`}>
            <h3 className={`${isMobile ? 'text-sm' : 'text-lg'} font-semibold text-slate-900 dark:text-white mb-2`}>Phân bổ</h3>
            <div className={`flex-1 relative ${isMobile ? 'min-h-[160px]' : 'min-h-[300px]'}`}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={statusData}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className={`${isMobile ? 'text-xl' : 'text-3xl'} font-bold text-slate-900 dark:text-white`}>{students.length}</span>
              </div>
            </div>
            <div className="mt-2 space-y-1">
              {statusData.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between text-[10px] md:text-sm group">
...
const StatCard: React.FC<{ title: string; value: number | string; change: string; icon: any; color: string; isNegative?: boolean; isMobile?: boolean }> = ({
  ...
  <div className={`bg-white dark:bg-dark-800 border border-slate-200 dark:border-dark-700 rounded-2xl ${isMobile ? 'p-2 py-3' : 'p-5'} shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group relative overflow-hidden`}>
    ...
    <div className="flex justify-between items-start relative z-10">
      <div>
        <p className={`text-slate-500 dark:text-slate-400 ${isMobile ? 'text-[10px]' : 'text-sm'} font-bold uppercase tracking-wider mb-0.5`}>{title}</p>
        <h4 className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold text-slate-900 dark:text-white`}>{value}</h4>
      </div>
      <div className={`${isMobile ? 'p-1.5' : 'p-2'} rounded-xl bg-opacity-20 ${color.replace('bg-', 'bg-opacity-20 ')}`}>
        <Icon className={`${isMobile ? 'w-3.5 h-3.5' : 'w-5 h-5'} ${color.replace('bg-', 'text-')}`} />
      </div>
    </div>
    <div className={`mt-2 md:mt-4 flex items-center gap-1.5 md:gap-2 relative z-10 ${isMobile ? 'text-[9px]' : 'text-xs'}`}>
